...
```{r, include = FALSE}
dat <- read_csv("data/220227_pineland_a_e/220227_pineland_ambient_O2.csv", skip = 1)
#head(dat)
#View(dat)
...
dat_2 <- dat %>%
  ...
  mutate(
    ...
    tank_channel == "Tank 1_4" ~ "10",
    tank_channel == "Tank 2_1" ~ "3",
    tank_channel == "Tank 2_2" ~ "4",
    tank_channel == "Tank 2_3" ~ "5",
    tank_channel == "Tank 2_4" ~ "6",
    tank_channel == "Tank 3_1" ~ "2",
    tank_channel == "Tank 3_2" ~ "9",
    tank_channel == "Tank 3_3" ~ "11",
    tank_channel == "Tank 3_4" ~ "12",
    tank_channel == "Tank 4_1" ~ "13",
    tank_channel == "Tank 4_2" ~ "14",
    tank_channel == "Tank 4_3" ~ "15",
...
dat_subset <- dat_2 %>%
  ...
  filter(delta_T_min > 11) #filtering out NAs and also test DO data from earlier that morning
#View(dat_subset)
...
ggsave("plots/220227_pineland_ambient_enriched/220227_pineland_ambient_DO_conc_plot.png", plot = DO_conc_plot, width = 10, height = 7)
...
ggsave("plots/220227_pineland_ambient_enriched/220227_pineland_ambient_temp_plot.png", plot = temp_plot, width = 10, height = 7)
...
ggsave("plots/220227_pineland_ambient_enriched/220227_pineland_ambient_DO_mass_plot.png", plot = DO_mass_plot, width = 10, height = 7)
...
htmlwidgets::saveWidget(as_widget(p), "plots/220227_pineland_ambient_enriched/220227_pineland_ambient_DO_mass_plot.html")
...
## the linear portion of the NEP curve is roughly from minute 44 to minute 127 ##

pnld_a_3 <- dat_full %>%
  filter(chamber == "3" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_3 <- dat_full %>%
  filter(chamber =="3" & delta_T_min >= 147)
...
pnld_a_4 <- dat_full %>%
  filter(chamber == "4" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_4 <- dat_full %>%
  filter(chamber =="4" & delta_T_min >= 147)
...
pnld_a_5 <- dat_full %>%
  filter(chamber == "5" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_5 <- dat_full %>%
  filter(chamber =="5" & delta_T_min >= 147)
...
pnld_a_6 <- dat_full %>%
  filter(chamber == "6" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_6 <- dat_full %>%
  filter(chamber =="6" & delta_T_min >= 147)
...
pnld_a_1 <- dat_full %>%
  filter(chamber == "1" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_1 <- dat_full %>%
  filter(chamber =="1" & delta_T_min >= 127)
...
pnld_a_7 <- dat_full %>%
  filter(chamber == "7" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_7 <- dat_full %>%
  filter(chamber =="7" & delta_T_min >= 127)
...
pnld_a_8 <- dat_full %>%
  filter(chamber == "8" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_8 <- dat_full %>%
  filter(chamber =="8" & delta_T_min >= 147)
...
pnld_a_8_plot
```
##### clipped ER portion of the data to minute 147 since this chamber did not show a pronouced ER signal at the beginning of the dark portion.
...
pnld_a_10 <- dat_full %>%
  filter(chamber == "10" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_10 <- dat_full %>%
  filter(chamber =="10" & delta_T_min >= 127)
...
pnld_a_2 <- dat_full %>%
  filter(chamber == "2" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_2 <- dat_full %>%
  filter(chamber =="2" & delta_T_min >= 147)
...
pnld_a_2_plot
```
##### clipped ER portion of the data to minute 147 since this tank had a temperature change that affected DO measurements ~ minute `140.
...
pnld_a_9 <- dat_full %>%
  filter(chamber == "9" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_9 <- dat_full %>%
  filter(chamber =="9" & delta_T_min >= 147)
...
pnld_a_11 <- dat_full %>%
  filter(chamber == "11" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_11 <- dat_full %>%
  filter(chamber =="11" & delta_T_min >= 147)
...
pnld_a_12 <- dat_full %>%
  filter(chamber == "12" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 147.
pnld_a_12 <- dat_full %>%
  filter(chamber =="12" & delta_T_min >= 147)
...
pnld_a_13 <- dat_full %>%
  filter(chamber == "13" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_13 <- dat_full %>%
  filter(chamber =="13" & delta_T_min >= 127)
...
pnld_a_14 <- dat_full %>%
  filter(chamber == "14" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_14 <- dat_full %>%
  filter(chamber =="14" & delta_T_min >= 127)
...
pnld_a_15 <- dat_full %>%
  filter(chamber == "15" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_15 <- dat_full %>%
  filter(chamber =="15" & delta_T_min >= 127)
...
pnld_a_16 <- dat_full %>%
  filter(chamber == "16" & delta_T_min >= 44 & delta_T_min <= 127)
...
## now measuring ER from the period after the NEP portion of the incubation (oxygen curve stabilizes past minute 127.
pnld_a_16 <- dat_full %>%
  filter(chamber =="16" & delta_T_min >= 127)
...
write_csv(dat_final, "data/220227_pineland_a_e/220227_pnld_ambient_rates.csv")
```
